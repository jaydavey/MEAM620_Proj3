function [path, cost] = dijkstra(map, start, goal, astar)
...
%   row is start and the last row is goal.  If no patha is found, patha is a
%   0-by-3 matrix.  Consecutive points in patha should not be farther apart than
%   neighboring cells in the map (e.g.., if 5 consecutive points in patha are
%   co-linear, don't simplify patha by removing the 3 intermediate points).
%
%   patha = DIJKSTRA(map, start, goal, astar) finds the patha using euclidean
%   distance to goal as a heuristic if astar is true.
%
%   [patha, NUM_EXPANDED] = DIJKSTRA(...) returns the patha as well as
...
start_grid_x = floor((start(1)/1) - (bound_xi/1)) + 1;
start_grid_y = floor((start(2)/1) - (bound_yi/1)) + 1;
start_grid_z = 1; %2D

goal_grid_x = floor((goal(1)/1) - (bound_xi/1)) + 1;
goal_grid_y = floor((goal(2)/1) - (bound_yi/1)) + 1;
goal_grid_z = 1; %2D
...
path = constructpatha(prev,start_i,goal_i); %Commented out for project 3

% [a,b,c] = ind2sub(map_sizes, pathaa);
% 
% if(~isempty(patha))
...
%function constructpatha(prev,start,goal);
%This function constructs the part from start to goal given an Nx1 vector 
%that states the predecesor of a node on a patha.
function patha = constructpatha(prev,start,goal)
